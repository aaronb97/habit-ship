...
export function createTiltShiftPasses(
  ...
  const apply = (p: Partial<TiltShiftParams>) => {
    ...
    (passH as unknown as { uniforms: typeof shaderH.uniforms }).uniforms.feather.value =
      next.feather;

    (passV as unknown as { uniforms: typeof shaderV.uniforms }).uniforms.feather.value =
      next.feather;
